...
\item atomic writing process with dynamic task partitioning
...
A coloring algorithm can be used as well and is similar to implement.
...
to an existing color has to have zero elements on the positions $A_{jk}$ for
...
The improvement of speed by parallelization is given at these steps (transposed
 multiplication and Gauß-Seidel). The coloring
for both steps has to be done separately for every matrixdimension,
 but can be used in every step of the AMG.

Figure \ref{figure:sequ} shows a sequential call of AMG, whereas figure \ref{figure:par}
shows a parallel call of AMG. Only one thread is working in the sequential part
compared to 24 threads in the parallel.

In figure \ref{figure:par} and \ref{figure:step} the red blocks mark a task of
GSSmooth, the purple ones are the projection and prolongations and the green
ones mark GSSmoothBack. In figure \ref{figure:step} a single step of the AMG
is shown. The large matrix needs more colors and time to perform a GSSmoot-step.
This can be seen at the red blocks at the beginning (only one color is calculated at 
the same time, but its rows can be passed to various threads. The blocks are getting
 smaller because less rows have the specific color.
...
Because of the matrix storage and the specific structur of the Vector-server its
two processors have different speed of writing and reading at parts of the matrix. Due
to this some colors are faster calculated by the first half of threads, and others by the
other half. This comes up with the coloring process. Iteration through the matrix will color
