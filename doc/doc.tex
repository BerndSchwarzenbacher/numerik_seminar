...
To solve big systems of linear equations in form of sparse positive-definite
matrices arising from finite-element discretization of partial differential
equations, the conjugate gradient method is the most used iterative method.
...
\lstset{language=C++, numbers=left, captionpos=t, breaklines=true,
        caption={Static balancing in the transposed vector multiplication. The seperation of the matrix 
is calculated based on the number of non-zero elements in the matrix. Each thread has to do nearly 
the same amount of work. },label=lst:tranbal}
\begin{lstlisting}
...
//find the next possible color
    ...
    cntcol = 0;
    for (int row = 0; row < height; ++row)
      coloring_[row_color[row]][cntcol[row_color[row]]++] = row;   //construct colors to row mapping
...
\begin{lstlisting}
  ...
  GSSmooth (BaseVector & x, const BaseVector & b) const
  {
    ...
    for (int j = 0; j < this->coloring_.Size(); ++j)  //iterate sequential over colors
    {
      auto color = this->coloring_[j];
      ParallelFor( this->balancing_[j],   //task-manager based parallelization
...
\begin{lstlisting}
  ...
  GSSmoothBack (BaseVector & x, const BaseVector & b) const
  {
    ...
    for (int j = this->coloring_.Size()-1; j >= 0; --j)   //iterate sequential over colors
    {
      auto color = this->coloring_[j];
      ParallelFor( this->balancing_[j],   //task-manager based parallelization
        [this, fx, fb, color] (int i)
